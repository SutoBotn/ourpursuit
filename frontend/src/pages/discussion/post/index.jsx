...
const Post = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const response = await axios.get(`/api/posts/${category}/${id}`)
    ...
    const saved = async () => {
      try {
        ...
        const response = await axios.get(`/api/posts/${id}/check-saved/`, { 
          headers: {
            'Authorization': `Bearer ${token}`
          },
        })
  ...
  const handleCommentSubmit = async () => {
    try {
      ...
      const response = await axios.post(`/api/posts/${category}/${id}/create-comment/`, {
        text: commentText
      }, {
        headers: {
          'Authorization': `Bearer ${token}`
      ...
      if (response){
        ...
        const updatedPostResponse = await axios.get(`/api/posts/${category}/${id}`);
  ...
  const handleSavePost = async () => {
    try {
      ...
      const response = await axios.post(`/api/posts/${id}/save-post/`, null, {
        headers: {
          'Authorization': `Bearer ${token}`,
